...
class EdiblesSpectrum:
    ...
    def statusReport(self):
        ...
        print("Reference frame: {ref_frame}".format(ref_frame=self.reference_frame))
    ...
    def baryCorrection(self):
        assert self.bary_corr is True, "Already in barycentric reference frame"

        if self.velocity_center is None:
            self.wave_working = DataHandling.velocityShift(self.wave_working, self.v_bary)
        else:
            self.wave_working = self.wave_working + self.v_bary
    ...
    def __getMinMax__(self, xmin, xmax):
        assert xmin < xmax, "xmin must be smaller than xmax!"
        if not DataHandling.within_boundaries(xmin,np.min(self.wave_working),np.max(self.wave_working))[0]:
            xmin = np.min(self.wave_working)
            print("Warning, x_min outside working region, reset to current min")

        if not DataHandling.within_boundaries(xmax,np.min(self.wave_working),np.max(self.wave_working))[0]:
    ...
    def cutSpectrum(self, xmin=None, xmax=None, center=None, span=None):
        ...
        (xmin, xmax) = self.__getMinMax__(xmin, xmax)
    ...
    def addMask(self, n=None):
        ...
        masked = DataHandling.within_boundaries(self.wave_working, boundary_left, boundary_right)
        idx_mask = [i for i in range(len(self.wave_working)) if masked[i] == 1]

        fig2, ax = plt.subplots(1,1)
        ax.plot(self.wave_working, self.flux_working)
        idx_mask2 = DataHandling.continous_idx(idx_mask)
    ...
    def fitContinuum(self, mode='spline', n=3,
        ...
        (cont, idx) = DataHandling.iterate_continuum(data_tuple,mode=mode, n=n,
        ...
        if not silence:
            ...
            # highlights
            idx2 = DataHandling.continous_idx(idx)
    ...
    def estimateSNR(self):
        ...
        mask[mask_idx] = 1
        data_tuple = (self.wave_working, self.flux_working)
        (cont, idx) = DataHandling.iterate_continuum(data_tuple, mode='polynomial', n=0, min_sigma = 0.1, mask=mask)
