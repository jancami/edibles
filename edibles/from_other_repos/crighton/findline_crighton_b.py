...
def find_edges_true_regions(condition):
    """ Finds the indices of edges of contiguous regions in an array
    that satisfy condition.

    Examples
    --------
    >>> a = np.array([3,0,1,4,6,7,8,6,3,2,0,3,4,5,6,4,2,0,2,5,0,3])
    >>> list( zip(*find_edges_true_regions(a > 2)) )
...
def find_ew_per_resel(wa, fl, er, co, resolution, weighted_aperture=True,
    ...
    mode : 'em' or 'abs'
        find absorption or emission lines.  Not yet implemented.
    ...
    if mode == 'abs':
        ...
        raise ValueError("mode keyword must be 'abs' or 'em' (%s)" % mode)
...
def __remove_close(indices, signif, minsep, nmin=1, joinfactor=1.2):
    ...
    for i1,i2 in zip(indices[1:-1], indices[2:]):
        ...
        # above significance of nmin region, (2) a more significant
        # component than i1 exists and (3) i1 is not a well-defined
        # component, then drop it.
...
def find_feature_edges(centres, above_nmin, signif):
    ...
    for i0,i1 in zip(ind0,ind1):
        ...
        for j in range(1, len(cen)-1):
            ...
            feature.append([left, c0, right, 1])
            #pl.vlines([left[-1],right[-1]], 0, 10)
            #pl.vlines([c0], 0, 10, colors='r')
            #raw_input()
        left = right
        feature.append([left, cen[-1], i1+1, 1])
        #pl.vlines([left[-1],right[-1]], 0, 10)
...
def find_feature_indices(ew, ew_err, fwhmpix, ndetect=5, nmin=1,
    ...
    lines = __remove_close(lines, signif, fwhmpix, joinfactor=joinfactor)
    left,indices,right,blended = find_feature_edges(
        lines, (signif >= nmin) & good, signif)

    return zip(left,lines,right,blended)
...
def findlines(wa, fl, er, co, resolution, ndetect=5, mode='abs',
    ...
    sp = find_ew_per_resel(wa, fl,er,co, resolution,
                           weighted_aperture=1, mode=mode)
    ...
    features = np.rec.fromarrays(
        [num, ewidth, ewidth_sig, fwhm, redshifts, idens, sp.wa[ind0],
         centres, sp.wa[ind2], ind[0], ind[1], ind[2], blended],
        names='num,ew,ewer,fwhm,z,iden,wa0,wac,wa1,i0,ic,i1,blend')
