...
edr5_dir = DATADIR
spec_dir = DATADIR / 'combined'
file_list = list(spec_dir.glob('*.fits'))
obs_log_path = files('edibles') / 'data/DR5_ObsLog_combined.csv'
...
for file in file_list:
    ...
    object_name = object_name.replace(' ', '').replace(' ', '').replace('HD', 'HD ')
    ra = hdr['RA']
    dec = hdr['DEC']
    ...
    out_s = pd.Series({'Object': object_name, 'RA': ra, 'DEC': dec, 'DateObs': date_obs, 'Setting': wave_setting, 'WaveMin': wave_min, 'WaveMax': wave_max, 'Filename': file_name})
    out_df = pd.concat((out_df, out_s), axis=1, ignore_index=True)

out_df = out_df.T.sort_values(by='Object')
