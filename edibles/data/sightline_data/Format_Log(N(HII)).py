...
"""
This routine will parse all the sources of Log(N(H2)) data and process them to produce the standard formatted output tables required for the oracle.
The Log(N(H2)) values are listed in 1 different file(s), and represent 1 different source(s) (in order of preference):
...
for objloop in range(n_obj):
    ...
    if len(match) != 0:
        this_value = match['log(NH2)'].values[0]
        if math.isnan(this_value):
            pass
        else:
            this_value_upper = match['Upper_log(NH2)'].values[0]
            
            unc_up= np.around(this_value_upper - this_value,decimals = 2)
            this_value_lower = match['Lower_log(NH2)'].values[0]
...
df_out_print.to_csv('Formatted_LogN(H2).csv', index=False, na_rep='NaN')
