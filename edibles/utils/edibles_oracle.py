...
class EdiblesOracle:
    ...
    def _getObsListFilteredByObsLogParameters(self, object=None, Wave=None, WaveMin=None, WaveMax=None,
                                              MergedOnly=False, OrdersOnly=False, closest_order=False):
    ...
    def FilterEngine(self, object, log, value, unc_lower, unc_upper, reference_id, filtername=None):
        ...
        print('getFilteredObslist: Filter=', filtername, '; found a total of ', bool_object_matches.sum(),
              ' object match(es).')
        print('getFilteredObslist: Filter=', filtername, '; found a total of ', bool_value_matches.sum(),
              ' parameter match(es).')
        print('getFilteredObslist: Filter=', filtername, '; found a total of ', bool_combined_matches.sum(),
              ' combined match(es).')
    ...
    def getFilteredObjects(self, object=None, Wave=None, \
        ...
        if (SpType or SpType_min or SpType_max or SpType_reference) is not None:
            print("SP_TYPE")
            matching_objects_sptype = self.FilterEngine(object, self.sptypelog, SpType, SpType_min, SpType_max,
                                                        SpType_reference)
        else:
        ...
        if (LogNHI or LogNHI_min or LogNHI_max or LogNHI_reference) is not None:
            print("LogN(HI)")
            matching_objects_lognhi = self.FilterEngine(object, self.nhilog, LogNHI, LogNHI_min, LogNHI_max,
                                                        LogNHI_reference)
        else:
        ...
        if (LogNHII or LogNHII_min or LogNHII_max or LogNHII_reference) is not None:
            print("LogN(HII)")
            matching_objects_lognhii = self.FilterEngine(object, self.nhiilog, LogNHII, LogNHII_min, LogNHII_max,
                                                         LogNHII_reference)
        else:
        ...
        common_objects_set = set(search_list).intersection(ebv_objects.to_list(), sptype_objects.to_list(),
                                                           lognhi_objects.to_list(), lognhii_objects.to_list(),
                                                           fh2_objects.to_list(), rv_objects.to_list(),
                                                           av_objects.to_list())
    ...
    def getFilteredObsList(self, object=None, Wave=None, MergedOnly=False, OrdersOnly=False, \
        ...
        FilteredObsList = self._getObsListFilteredByObsLogParameters(object=common_objects_list, Wave=Wave,
                                                                     WaveMin=WaveMin, WaveMax=WaveMax,
                                                                     MergedOnly=MergedOnly, OrdersOnly=OrdersOnly,
...
if __name__ == "__main__":
    ...
    # EXAMPLE 0: Get all objects that match a set of filter specifications.
    # Should we perhaps rename the methods to be consistent?
    ObjectList = pythia.getObjects(EBV_min=0.2, EBV_max=0.8)
    ...
    # EXAMPLE 2: Get all observations matching EBV specs, using SIMBAD values.
    List = pythia.getFilteredObsList(EBV_min=0.2, EBV_max=0.8, EBV_reference=4)
    print("Example 2. Results from getFilteredObsList: ")
    print(List)
