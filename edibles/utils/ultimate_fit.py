...
def fit_multi_voigt_absorptionlines(wavegrid=np.array, ydata=np.array, restwave=np.array, f=np.array, gamma=np.array, 
             b=np.array, N=np.array, v_rad=np.array, v_resolution=0., n_step=0, std_dev = 1,map =np.array):
    """
...
if __name__ == "__main__":
    ...
    show_example = 4  # <---------------- GIVE YOUR INPUT HERE ************************
    ...
    if show_example == 1:
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= [0.4982,0.2491,0.4982,0.2491], restwave=restwave, gamma=gamma, 
                            b=[2,2], N=[0.00073e13,0.00073e13/18], v_rad=-7.1, v_resolution=3, n_step=25, map =[0,0,1,1],std_dev= 0.0035554091884622933)
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b, N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =[0,0],std_dev= 0.0035554091884622933)
        fitresult.params.pretty_print()
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b, N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =[0,1],std_dev= 0.0015)
        fitresult.params.pretty_print()
        ...
        fitresult = fit_multi_voigt_absorptionlines(wavegrid=wave, ydata= flux,f= f, restwave=lambda0, gamma=gamma, 
                               b=b, N=N, v_rad=v_rad, v_resolution=v_resolution, n_step=25, map =map ,std_dev= std_dev)
        fitresult.params.pretty_print()
