...
def VoigtOpticalDepth(wave, lambda0=0., b=0., N=0., f=0., gamma=0., v_rad=0.): 
    ...
    Returns:
        tau (float64):             Optical Depth at wave. 
    ...
    ThisVoigtProfile = VoigtProfile(nu-nu0, sigma, gamma_voigt)
...
def VoigtAbsorptionLine(wavegrid, lambda0=0., f=0., gamma=0., b=0., N=0., \
                        v_rad=0., v_resolution=0.):
    """
    Function to return a complete Voigt Absorption Line Model, smoothed to the specified resolution and
    resampled to the desired wavelength grid.  
    This can in fact be a set of different absorption lines -- same line, different cloud components or
    different line for single cloud component. 
    ...
    #    --> Each component represents a cloud; use the same spectral line parameters for each component. 
    #        We will set that up here, and then do a recursive call to VoigtAbsorptionLine. 
    # 2. Multiple lines, but a single component. 
    #    --> E.g. the Na doublet lines. For each line component, we will use the same cloud components. 
    #        We will set that up here, and then do a recursive call to VoigtAbsorptionLine. 
    # 3. Multiple lines, and multiple components. 
    #    Here, we consider 2 different cases: 
    #    A) If n_lines == n_components, we will treat each combination as a unique, single line. We could
    #       could have gotten here from the recursive calls in 1. and 2. Exception: if the keyword /MultiCloud
    #       is set. In that case we will proceed as in B). 
    #    B) If n_lines <> n_components, we will interpret this as meaning that each component will produce
    #       each of the lines. This means we now have to create n_lines * n_components entries for the 
    #       recursive call. We will set that up here. 

    if (n_lines == 1) & (n_components !=1): 
        # Case 1 from above. Replicate all the line parameters for each of the components, and issue the
        # call to self. 
        ...
        interpolated_model = VoigtAbsorptionLine(wavegrid, lambda0=lambda0_use, f=f_use, gamma=gamma_use, b=b, N=N,  \
                                                 v_rad=v_rad,v_resolution=v_resolution)
    elif (n_components == 1) & (n_lines != 1):
        # Case 2 from above. Replicate all the sightline parameters for each of the lines, and issue the
        # call to self. 
        b_use = np.repeat(b,n_lines)
        N_use = np.repeat(N,n_lines)
        v_rad_use = np.repeat(v_rad,n_lines)
        interpolated_model = VoigtAbsorptionLine(wavegrid, lambda0=lambda0, f=f, gamma=gamma, b=b_use, N=N_use,  \
                                                 v_rad=v_rad_use,v_resolution=v_resolution)
        ...
        bluewaves = lambda0_array * (1. + (v_rad_array-20.*b_array)/cst.c.to("km/s").value)
        redwaves = lambda0_array * (1. + (v_rad_array+20.*b_array)/cst.c.to("km/s").value)
        ...
        refgrid = minwave * (1. + np.arange(n_v) * v_stepsize / cst.c.to("km/s").value)
        allcomponents = np.zeros(shape=(n_v, n_lines))

        for lineloop in range(n_lines):
            v_limits = [-20.*b_array[lineloop], 20.*b_array[lineloop]]
            dv = np.arange(start=v_limits[0], stop=v_limits[1], step=v_stepsize)  # in km/s    
            thiswavegrid = lambda0_array[lineloop] * (1. + dv/cst.c.to("km/s").value)
            tau = VoigtOpticalDepth(thiswavegrid, lambda0=lambda0_array[lineloop], b=b_array[lineloop], N=N_array[lineloop], \
                                    f=f_array[lineloop], gamma=gamma_array[lineloop], v_rad=v_rad_array[lineloop])
            # Shift to the proper wavelength given the radial velocity
            vel = dv + v_rad_array[lineloop]
            thiswavegrid = lambda0_array[lineloop] * (1. + vel/cst.c.to("km/s").value)
        ...
        interpolationfunction = interp1d(refgrid, gauss_smooth, kind='cubic', bounds_error=False, fill_value = (1, 1))
        ...
        print("VoigtAbsorptionLine Panic: This option has not been implemented yet.... ")
...
if __name__ == "__main__":
    ...
    """
    This is a series of examples / tests to see if the implementation of the various Voigt functions has been done correctly. 
    It includes tests for the basic Voigt function, as well as for the various forms of the normalized Voigt profiles. 
    One test aims to reproduce the high-resolution profile for the K line of omi Per (form Welty et al.).
    """
    show_example = 2
    ...
    if (show_example == 1): 
        ...
        ##
        #############################################################
        wavegrid = np.arange(1000) * .01 + 5000
        AbsorptionLine = VoigtAbsorptionLine(wavegrid, lambda0=5003., b=0.75, N=1e11, f=1, gamma=1e7, \
                                         v_rad=-10.,v_resolution=0.26)
        ...
        ##
        #############################################################
        # This, in fact, is reproducing Dan Welty's high-resolution observations of the K line of o Per. 
        folder = Path(PYTHONDIR+"/data")
        filename=folder /"voigt_benchmarkdata"/"omiper.m95.7698.txt"
        Headers = ["Wavelength", "Normfluxval"]
        omiper_data=pd.read_csv(filename,delim_whitespace=True,skiprows=[0],header=None,names=Headers,engine='python')
        ...
        v_rad = np.array([10.50, 11.52, 13.45, 14.74, 15.72]) + .1          # Radial velocity of cloud in km/s
        v_resolution = 0.56    # Instrumental resolution in km / s

        AbsorptionLine = VoigtAbsorptionLine(omiper_data["Wavelength"], lambda0=lambda0, b=b, N=N, f=f, gamma=gamma, \
                                             v_rad=v_rad,v_resolution=v_resolution)
 
        plt.plot(omiper_data["Wavelength"], omiper_data["Normfluxval"], marker='D', fillstyle='none', color='black')
        plt.gca().get_xaxis().get_major_formatter().set_useOffset(False)
        ...
        List=pythia.getFilteredObsList(object=["HD 145502"],MergedOnly=True,Wave=3302.)
        ...
        AbsorptionLine = VoigtAbsorptionLine(wave, lambda0=lambda0, b=b, N=N, f=f, gamma=gamma, \
                                             v_rad=v_rad,v_resolution=v_resolution)
        plt.plot(wave, flux)
        plt.gca().get_xaxis().get_major_formatter().set_useOffset(False)
