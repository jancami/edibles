...
for on in object_names:
    ...
    def get_rotational_spectrum(xx, B, T, delta_B, zeta, sigma, origin):
        ...
        plt.plot(simu_waveno, simu_intenisty, color = 'red', label = on)
        plt.xlim(-7.5, 7.5)
        ...
        plt.title('Temperature = ' + '{:.4f}'.format(T) + '  K  ground_B =  ' + '{:.4f}'.format(ground_B) + ' cm-1  Delta_B = ' + '{:.4f}'.format(delta_B) + '    $\sigma$ = ' + '{:.4f}'.format(sigma) +    '    zeta = ' +  '{:.4f}'.format(zeta) + '   origin = ' + '{:.4f}'.format(origin) + '   Red. chi2 = ' + '{:.4f}'.format(redchi)) 
        #plt.xlim(-5, 3)
