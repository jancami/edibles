...
#
# EXMPLE:
#     plot HD22951 in velocity and apply the Barycentric correction
#     ipython edibles_plt.py HD22951 bar x:5770-5810 vel:5780
#
# Author:
#        Amin Farhang
#
...
if len(xrng) != 0:
    if len(xrng.split('-')) == 2:
        xrng = xrng[2:]
        xmin = float(xrng.split('-')[0])
        xmax = float(xrng.split('-')[1])
    if len(xrng.split('-')) == 1:
        xrng = xrng[2:]
        xmin = float(xrng.split('-')[0]) - 10.0
        xmax = float(xrng.split('-')[0]) + 10.0
else:
...
# determine y range
idx = [i for i, e in enumerate(w_air) if e > xmin and e < xmax]
flx_new = []
for i in idx:
    flx_new.append(flx[i])
ymin = min(flx_new)
ymax = max(flx_new)
